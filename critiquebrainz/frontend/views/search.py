...
def search_wrapper(query, type, offset=None):
    if query:
        if type == "artist":
            count, results = musicbrainz.ws.search_artists(query, limit=RESULTS_LIMIT, offset=offset)
        elif type == "event":
            count, results = musicbrainz.ws.search_events(query, limit=RESULTS_LIMIT, offset=offset)
        elif type == "place":
            count, results = musicbrainz.ws.search_places(query, limit=RESULTS_LIMIT, offset=offset)
        elif type == "release-group":
            count, results = musicbrainz.ws.search_release_groups(query, limit=RESULTS_LIMIT, offset=offset)
        else:
            count, results = 0, []
    else:
        count, results = 0, []
...
def selector():
    ...
    if artist or release_group:
        count, results = musicbrainz.ws.search_release_groups(artist=artist, release_group=release_group,
                                                           limit=RESULTS_LIMIT)
    elif event:
        count, results = musicbrainz.ws.search_events(event, limit=RESULTS_LIMIT)
    elif place:
        count, results = musicbrainz.ws.search_places(place, limit=RESULTS_LIMIT)
    else:
        count, results = 0, []
...
def selector_more():
    ...
    if type == 'release-group':
        ...
        count, results = musicbrainz.ws.search_events(event, limit=RESULTS_LIMIT, offset=offset)
    elif type == 'place':
        count, results = musicbrainz.ws.search_places(place, limit=RESULTS_LIMIT, offset=offset)
    else:
        count, results = 0, []
