...
def to_dict_relationships(data, source_obj, relationship_objs):
    """Convert relationship objects to dictionary.
    ...
    if relationship_objs:
        print(relationship_objs)
        data['relationships'] = {}

    for type in ENTITY_TYPES:
        relation = '{type}-rels'.format(type=type)
        if relation in relationship_objs:
            ...
            for obj in relationship_objs[relation]:
                ...
                link_data[type] = TO_DICT_ENTITIES[type](get_link_target(obj, source_obj))
                data['relationships'][relation].append(link_data)
